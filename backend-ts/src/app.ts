...
export const PORT_SOCKET_CLIENT = 3000;
const PORT = 5000;

const { server, io } = Socket(app); //initialize the socket server

ScheduleSync(async (data: any) => {
  io.emit("fetch-live-exchange", data); //fetch-live-exchange
  let savedExchange = await CurrencyExchangeService.fetch(TYPE_ALL,'','',0,true);
  io.emit("fetch", savedExchange);
});
